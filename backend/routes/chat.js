...
router.get('/conversations', (req, res) => {
  ...
  const enriched = rows.map((c) => {
    if (!c.is_group) {
      ...
      return { ...c, name: other?.username, avatar_url: other?.avatar_url, other_user_id: other?.id };
    }
    return c;
  });
...
router.post('/conversations', (req, res) => {
  ...
  insertMember.run(id, req.user.id, 1);
  member_ids.forEach((uid) => insertMember.run(id, uid, 0));
...
router.post('/conversations/:id/messages', (req, res) => {
  ...
  const id = uuid();
  db.prepare(
    'INSERT INTO messages (id, conversation_id, sender_id, content, media_url) VALUES (?, ?, ?, ?, ?)'
  ...
  otherMembers.forEach((m) =>
    notify(io, { user_id: m.user_id, actor_id: req.user.id, type: 'message', conversation_id: req.params.id, message_id: id })
  );
