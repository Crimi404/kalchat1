...
router.post('/', (req, res) => {
  ...
  db.prepare(
    `INSERT INTO stories (id, user_id, media_url, caption, shared_from_id, expires_at)
     VALUES (?, ?, ?, ?, ?, datetime('now', '+${LIFETIME_HOURS} hours'))`
...
router.post('/:id/share', (req, res) => {
  const original = db.prepare('SELECT * FROM stories WHERE id = ?').get(req.params.id);
...
router.post('/:id/like', (req, res) => {
  ...
  const count = db.prepare('SELECT COUNT(*) AS n FROM story_likes WHERE story_id = ?').get(req.params.id).n;
...
router.post('/:id/comments', (req, res) => {
  ...
  const id = uuid();
  db.prepare(
    'INSERT INTO story_comments (id, story_id, user_id, content) VALUES (?, ?, ?, ?)'
