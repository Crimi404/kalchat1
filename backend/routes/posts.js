...
router.post('/', (req, res) => {
  ...
  res.status(201).json(getPostById(id, req.user.id));
});
...
router.post('/:id/share', (req, res) => {
  ...
  const id = uuid();
  db.prepare(
    'INSERT INTO posts (id, user_id, content, media_url, shared_from_id) VALUES (?, ?, ?, ?, ?)'
  ).run(id, req.user.id, original.content, original.media_url, original.id);

  notify(req.app.get('io'), { user_id: original.user_id, actor_id: req.user.id, type: 'share', post_id: original.id });
...
router.post('/:id/like', (req, res) => {
  ...
  if (already) {
    ...
    notify(req.app.get('io'), { user_id: post.user_id, actor_id: req.user.id, type: 'like', post_id: post.id });
  }

  const count = db.prepare('SELECT COUNT(*) AS n FROM post_likes WHERE post_id = ?').get(req.params.id).n;
...
router.post('/:id/bookmark', (req, res) => {
  ...
  if (already) {
    db.prepare('DELETE FROM post_bookmarks WHERE post_id = ? AND user_id = ?').run(req.params.id, req.user.id);
  } else {
    db.prepare('INSERT INTO post_bookmarks (post_id, user_id) VALUES (?, ?)').run(req.params.id, req.user.id);
  }
...
router.post('/:id/comments', (req, res) => {
  ...
  notify(req.app.get('io'), { user_id: post.user_id, actor_id: req.user.id, type: 'comment', post_id: post.id });
...
router.get('/', (req, res) => {
  ...
  const rows = db
    .prepare(
      `SELECT p.id, p.user_id, u.username, u.avatar_url, p.content, p.media_url, p.created_at,
...
function getPostById(id, viewerId) {
  return db
    .prepare(
      `SELECT p.id, p.user_id, u.username, u.avatar_url, p.content, p.media_url, p.created_at,
              0 AS like_count, 0 AS liked_by_me, 0 AS comment_count, 0 AS share_count,
              0 AS bookmarked_by_me, su.username AS shared_from_username
       FROM posts p
       JOIN users u ON u.id = p.user_id
       LEFT JOIN posts so ON so.id = p.shared_from_id
       LEFT JOIN users su ON su.id = so.user_id
